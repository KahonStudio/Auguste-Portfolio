...
export async function POST(request: Request) {
  ...
  const existing = await prisma.workingWithMeStep.findUnique({
    where: { id: data.id },
  });
  if (existing) {
  ...
  const count = await prisma.workingWithMeStep.count();
  const row = await prisma.workingWithMeStep.create({
    data: {
      id: data.id,
      title: data.title,
